require_relative 'support/helpers/helper.rb'
require_relative 'support/stubs_data/new_plan.rb'
...
RSpec.describe PagseguroRecorrencia do
  ...
  it 'has a version number' do
    expect(PagseguroRecorrencia::VERSION).not_to be nil
  end
  ...
  it 'when request all fields with empty :plan_charge return raise error' do
    ...
    expect { PagseguroRecorrencia.new_plan(tmp_payload) }.to raise_error
    expect { raise StandardError, '[VALUE_ERROR] :charge_type can only receive these values (:manual, :auto)'}.
      to raise_error(StandardError, '[VALUE_ERROR] :charge_type can only receive these values (:manual, :auto)')
  end
